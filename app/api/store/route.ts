...
export async function POST(req: Request) {
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS "Tracking-Midyear Changes 302 - Workplace Sim: QLE Consistency Rule" (
    ...
    const existingSession = await pool.query(
      `SELECT * FROM "Tracking-Midyear Changes 302 - Workplace Sim: QLE Consistency Rule" WHERE session_id = $1`,
      [sessionId]
    );
    ...
    if (existingSession.rows.length > 0) {
      ...
      const result = await pool.query(
        `UPDATE "Tracking-Midyear Changes 302 - Workplace Sim: QLE Consistency Rule" 
      ...
      // Create new session
      const result = await pool.query(
        `INSERT INTO "Tracking-Midyear Changes 302 - Workplace Sim: QLE Consistency Rule" (session_id, question, response, conversation_data, timestamp) 
         VALUES ($1, $2, $3, $4, $5) 
         RETURNING *`,
        [
          sessionId,
